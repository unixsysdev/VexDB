...
var (
	...
	ErrInvalidOffset     = errors.New("invalid vector offset")
...
func NewSegment(path string, vectorDim uint32, clusterID uint32, maxVectors int) *Segment {
	now := time.Now().Unix()
	...
	return &Segment{
		...
		footer:      &Footer{},
		index:       make([]*IndexEntry, 0, maxVectors),
...
func LoadSegment(path string) (*Segment, error) {
	...
	return NewSegment(path, 128, 0, DefaultMaxVectorsPerSegment), nil
}

// AddVector adds a vector to the segment
func (s *Segment) AddVector(vector *types.Vector) error {
	if s.isReadOnly {
	...
	if _, exists := s.vectors[vector.ID]; exists {
		return fmt.Errorf("vector with ID %d already exists", vector.ID)
	}
	
	s.vectors[vector.ID] = vector
	if vector.Metadata != nil {
		s.metadata[vector.ID] = vector.Metadata.Serialize()
	}
...
func (s *Segment) GetVector(vectorID uint64) (*types.Vector, error) {
	vector, exists := s.vectors[vectorID]
	if !exists {
...
func (s *Segment) RemoveVector(vectorID uint64) error {
	if s.isReadOnly {
		return ErrSegmentFull
	}
...
func (s *Segment) GetVectors() []*types.Vector {
...
func (s *Segment) GetVectorCount() int {
...
func (s *Segment) serializeHeader() ([]byte, error) {
...
func (s *Segment) deserializeHeader(data []byte) error {
...
func (s *Segment) serializeData() ([]byte, error) {
	buf := make([]byte, 0)
	...
	for _, vector := range s.vectors {
		...
		if metadata, exists := s.metadata[vector.ID]; exists {
		...
		if vectorHeader.MetadataSize > 0 {
			buf = append(buf, s.metadata[vector.ID]...)
		}
...
func (s *Segment) deserializeData(data []byte) error {
	offset := 0
...
func (s *Segment) serializeIndex() ([]byte, error) {
...
func (s *Segment) deserializeIndex(data []byte) error {
	if len(data)%IndexEntrySize != 0 {
		return ErrCorruptedData
	}
	
	s.index = make([]*IndexEntry, 0, len(data)/IndexEntrySize)
...
func (s *Segment) serializeFooter() ([]byte, error) {
...
func (s *Segment) deserializeFooter(data []byte) error {
...
func (s *Segment) GetVectorDim() uint32 {
...
func (s *Segment) GetModifiedAt() time.Time {
